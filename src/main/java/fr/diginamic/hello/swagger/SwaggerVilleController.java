...
public interface SwaggerVilleController {
    ...
    @Operation(
            summary = "📥 Import en lot de villes",
            description = """
            **Importe plusieurs villes simultanément.**
            
            Traitement transactionnel : soit toutes les villes sont créées, 
            soit aucune en cas d'erreur sur l'une d'entre elles.
            
            **Validation :**
            - Chaque ville respecte les règles métier
            - Pas de doublons dans le lot
            - Départements existants
            
            **Limitation :** Maximum 1000 villes par import.
            """,
            tags = {"🏙️ Villes", "📥 Import/Export"}
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "201",
                    description = "✅ Toutes les villes importées",
                    content = @Content(
                            mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = VilleDTO.class))
                    )
            ...
            @ApiResponse(
                    responseCode = "413",
                    description = "❌ Trop de villes à importer (> 1000)",
                    content = @Content()
            )
    })
    List<VilleDTO> importVilles(
            @Parameter(
                    description = "Liste des villes à importer",
                    required = true,
                    schema = @Schema(type = "array", maxLength = 1000)
            ) @RequestBody List<VilleDTO> villesDTO
    );

    @Operation(
            summary = "📤 Export de toutes les villes d'un département",
            description = """
            **Exporte la liste complète des villes d'un département.**
            
            Format JSON adapté pour sauvegardes, analyses externes,
            ou transferts entre systèmes.
            
            **Tri :** Par nom de ville (alphabétique).
            **Format :** Données complètes avec informations du département.
            """,
            tags = {"🏙️ Villes", "📤 Import/Export"}
    )
    @ApiResponses(value = {
    ...
    List<VilleDTO> exportVillesByDepartement(
            @Parameter(
                    description = "Code du département à exporter",
                    example = "34",
                    required = true
            ) @PathVariable String code
    );
